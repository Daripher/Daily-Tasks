...
@EventBusSubscriber
public class TaskEatFood implements ITask
{
	...
	@SubscribeEvent
	public static void onEntityUsedItem(LivingEntityUseItemEvent.Finish event)
	{
		...
		while (tasksIterator.hasNext())
		{
			...
			if (task instanceof TaskEatFood)
			{
				ItemStack result = event.getItem().copy();
				result.setCount(1);
				
				if (ItemStack.areItemStacksEqualUsingNBTShareTag(((TaskEatFood) task).item, result))
				{
